...
**Supabase Integration Status**: Phase 3 complete (full bidirectional sync). Anonymous auth and device fingerprinting ready. The progressStore automatically syncs to Supabase cloud on all state changes (WRITE) with 5-second throttling and AES encryption for sensitive data. On app start, it restores from cloud if cloud data is newer (READ). Cross-device data sync ready - will activate once Supabase database tables are created (Phase 2). Asset management is centralized via `assets.js` for optimized WebP images. Development and deployment use Vite.

...
**Layout Contract (Feb 2026 Refactor)**: Single scroll container architecture. `html`/`body`/`#root` are all `height: 100%; overflow: hidden` — they never scroll. The sole scroll container is `.app-root-container` with `flex: 1 1 0%; min-height: 0; overflow-y: auto; background: #0e2340`. CSS variables `--nav-height: 76px` and `--nav-total: calc(76px + env(safe-area-inset-bottom))` define nav clearance. Only `.app-root-container` has `padding-bottom: var(--nav-total)` — NO individual pages add nav clearance padding. BottomNav is `position: fixed; height: calc(76px + safe-area)` with `alignItems: flex-start; paddingTop: 8px` so buttons stay above the safe area. **Unified background `#0e2340`** defined ONCE on `.app-root-container` in App.css — NO page-level background overrides on wrapper divs. `html`/`body`/BottomNav also use `#0e2340` for consistency. No `100vh` usage anywhere — pages use `min-height: 100%` (relative to scroll container). `.screen` class has no `background-color` or `padding-bottom` — it only sets `min-height: 100%` and `overflow-x: clip`. Modals/popups may use their own darker backgrounds since they render as overlays.
...
**Profile Creation Architecture**: Profiles are created exclusively server-side — by the DB trigger on `auth.users` insert, and by the `create_profile_if_missing` SECURITY DEFINER RPC (called post-email-confirmation as a safe fallback). No `INSERT`/`UPSERT` into `profiles` from the frontend. The startup sequence involves session check, device ID check, then either onboarding or loading existing stores. No anonymous/hidden accounts are created at any point.

**Supabase Integration**: No silent account creation. `ensureSignedIn()` only checks for an existing session and returns null if none exists. `progressStore` automatically syncs to Supabase cloud with throttling and AES encryption. Cloud data is restored if newer on app start.

**Premium State (Mar 2026 Refactor)**: Single source of truth — `profiles.premium` (boolean). The store exposes only `premium: boolean`. Legacy fields `hasPremium`, `premiumStatus`, `premiumType`, `premiumActivatedAt`, and `_premiumMigrationV1` have been removed from state and are scrubbed from localStorage on first hydration (one-time migration reads and deletes them). All UI (`Profile.jsx`, `Pathway.jsx`, `Home.jsx`, `Challenge.jsx`, `GlobalEvents.jsx`) and all lock-gating logic reads `premium` directly. `loadFromSupabase` and `setFromCloudSync` both set only `premium`. IAP entitlement (`iq_iap_premium_entitlement`) is checked at hydration and cloud-sync time to prevent cloud `false` from revoking a locally-granted purchase. `[PREMIUM_FINAL]` logs mark every mutation point.

**Streak System (Feb 2026 Refactor)**: Local state uses `lastStreakDate` + `streak` fields (renamed from `lastCompletedActivityDate`). A `_localStreakTs` timestamp tracks when the local streak was last mutated. Cloud sync uses a merge guard: if `_localStreakTs > cloudUpdatedAt`, local streak data is preserved regardless of cloud values. The DB column remains `last_completed_activity_date` for backward compatibility. All streak mutations set `_localStreakTs = Date.now()`. Trace logs use `[IQ_STREAK_TRACE]` prefix. Vite `drop_console` is disabled to allow trace logs in production builds.

**Streak Validation (Mar 2026 Hardening)**: `checkStreakOnAppOpen()` is now `async` and runs in two places: (1) `Home.jsx` mount effect, (2) `App.jsx` `window focus` + Capacitor `appStateChange` listener — so it fires correctly on foreground resume. Date arithmetic uses `Date.UTC()` to avoid timezone drift. On shield auto-consumed: syncs `shield_count`, `streak`, `streak_active`, and `last_completed_activity_date` to Supabase then dispatches `MODAL_TYPES.STREAK_SAVED`. On streak broken: syncs `streak: 0` and `streak_active: false` to Supabase then sets `needsRepairPrompt: true` (triggers `MODAL_TYPES.REPAIR_STREAK` via Home.jsx effect). `recordDailyActivity` now also writes `streak_active` in its sync payload. **REQUIRED**: Run `supabase/migrations/add_streak_active.sql` in Supabase SQL Editor to add the `streak_active` boolean column to `profiles`.

**Email/Password Authentication System**: A hardened email/password authentication system with email confirmation and password reset. Key properties: (1) No pre-emptive `signOut()` before `signUp()`. (2) `signUp()` stores `desired_username`, `desired_handle`, `desired_avatar_index` in `auth.user_metadata` so profile creation never depends on localStorage. (3) Duplicate email detection checks `error.code` first (`user_already_exists`/`email_exists`), falls back to status+message. (4) `CheckEmailScreen` uses `exchangeCodeForSession()` for PKCE flows and `getSession()` for implicit flows — no timing delays. (5) If profile is missing post-confirmation, `create_profile_if_missing` RPC is called before proceeding. (6) `LoginPage` polls for profile up to 10× at 250ms before calling the RPC fallback. Deep linking handles native app email callbacks.

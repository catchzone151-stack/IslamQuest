...
**Layout Contract (Feb 2026 Refactor)**: Single scroll container architecture. `html`/`body`/`#root` are all `height: 100%; overflow: hidden` — they never scroll. The sole scroll container is `.app-root-container` with `flex: 1 1 0%; min-height: 0; overflow-y: auto`. CSS variables `--nav-height: 76px` and `--nav-total: calc(76px + env(safe-area-inset-bottom))` define nav clearance. Only `.app-root-container` has `padding-bottom: var(--nav-total)` — NO individual pages add nav clearance padding. BottomNav is `position: fixed; height: calc(76px + safe-area)` with `alignItems: flex-start; paddingTop: 8px` so buttons stay above the safe area. Background is `#0e2340` on `html`/`body` matching the BottomNav to prevent any bleed strip. No `100vh` usage anywhere — pages use `min-height: 100%` (relative to scroll container). `.screen` class has no `background-color` or `padding-bottom` — it only sets `min-height: 100%` and `overflow-x: clip`.

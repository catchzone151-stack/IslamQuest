const namesOfAllahLessonsFull = [
  {
    id: 1,
    title: "Allah ﷻ",
    arabic: "اللّٰه",
    meaning: "The One True God",
    description: [
      "Allah ﷻ is the only One who deserves all worship. Everything that exists belongs to Him.",
      "He created us, gives us life, and guides us. No one shares His power or His perfection.",
      "When we say Allah ﷻ, we mean the One who has every beautiful Name and perfect quality.",
      "Remembering Allah ﷻ keeps our hearts calm and helps us live in the way He loves."
    ],
    reflection: "How often do you remember Allah ﷻ during your day?",
    quizId: "allah_quiz"
  },
  {
    id: 2,
    title: "Ar-Rahmaan",
    arabic: "الرَّحْمَـٰن",
    meaning: "The Most Merciful",
    description: [
      "Ar-Rahmaan means that Allah ﷻ’s mercy is vast and covers all creation.",
      "He shows mercy to everyone—believers and non-believers alike.",
      "Every good thing you see around you is from His mercy.",
      "When we are gentle and kind, we are following the example of Allah ﷻ’s mercy."
    ],
    reflection: "How can you show mercy to someone today?",
    quizId: "ar_rahmaan_quiz"
  },
  {
    id: 3,
    title: "Ar-Raheem",
    arabic: "الرَّحِيم",
    meaning: "The Especially Merciful",
    description: [
      "Ar-Raheem means Allah ﷻ gives special mercy to those who believe in Him.",
      "He forgives them, guides them, and rewards them in this life and the next.",
      "Allah ﷻ’s mercy never runs out no matter how many times we make mistakes.",
      "We should thank Allah ﷻ for always being kind and loving towards us."
    ],
    reflection: "Who can you forgive to earn Allah ﷻ’s mercy?",
    quizId: "ar_raheem_quiz"
  },
  {
    id: 4,
    title: "Al-Malik",
    arabic: "الْمَلِك",
    meaning: "The King",
    description: [
      "Al-Malik means Allah ﷻ is the true King and Owner of everything.",
      "Nothing happens without His permission and control.",
      "He gives people power in this world, but He alone is the real King.",
      "Knowing Allah ﷻ is the King reminds us to stay humble and thankful."
    ],
    reflection: "How can you show respect to Allah ﷻ the King today?",
    quizId: "al_malik_quiz"
  },
  {
    id: 5,
    title: "Al-Maalik",
    arabic: "الْمَالِك",
    meaning: "The Owner",
    description: [
      "Al-Maalik means Allah ﷻ owns everything completely.",
      "He owns the heavens, the earth, and everything between them.",
      "Whatever we have belongs to Him, and He can give or take as He wills.",
      "We should never be proud of what we have, but use it in ways that please Allah ﷻ."
    ],
    reflection: "Do you thank Allah ﷻ for the things He lets you use?",
    quizId: "al_maalik_quiz"
  },
  {
    id: 6,
    title: "Al-Maleek",
    arabic: "الْمَلِيك",
    meaning: "The Supreme Sovereign",
    description: [
      "Al-Maleek means Allah ﷻ rules with perfect power and justice.",
      "He never forgets, never makes mistakes, and never becomes weak.",
      "All kings and leaders in this world will one day stand before Him.",
      "Remembering that Allah ﷻ is Al-Maleek helps us make fair and honest choices."
    ],
    reflection: "How can you act fairly like Allah ﷻ loves?",
    quizId: "al_maleek_quiz"
  },
  {
    id: 7,
    title: "Al-Quddoos",
    arabic: "الْقُدُّوس",
    meaning: "The Most Pure",
    description: [
      "Al-Quddoos means Allah ﷻ is completely pure and perfect.",
      "He is free from any weakness, mistake, or need.",
      "Everything He does is right, wise, and beautiful.",
      "We should try to keep our hearts and actions pure for His sake."
    ],
    reflection: "What can you do today to keep your heart pure?",
    quizId: "al_quddoos_quiz"
  },
  {
    id: 8,
    title: "As-Salaam",
    arabic: "السَّلَام",
    meaning: "The Source of Peace",
    description: [
      "As-Salaam means Allah ﷻ is the One who gives peace and safety.",
      "All comfort and calmness come from Him alone.",
      "In Jannah, the believers will hear Him say 'Peace!' as a greeting.",
      "We can spread peace by being kind and saying 'As-Salaamu ʿAlaykum'."
    ],
    reflection: "Who can you greet with peace today?",
    quizId: "as_salaam_quiz"
  },
  {
    id: 9,
    title: "Al-Mu'min",
    arabic: "الْمُؤْمِن",
    meaning: "The Giver of Faith and Security",
    description: [
      "Al-Mu’min means Allah ﷻ gives belief and safety to His servants.",
      "He protects those who trust Him and keeps their hearts firm.",
      "He is the One who guides us to the truth and keeps us safe from harm.",
      "When you feel scared, remember that Allah ﷻ is the best protector."
    ],
    reflection: "When do you feel safest with Allah ﷻ’s protection?",
    quizId: "al_mumin_quiz"
  },
  {
    id: 10,
    title: "Al-Muhaymin",
    arabic: "الْمُهَيْمِن",
    meaning: "The Guardian",
    description: [
      "Al-Muhaymin means Allah ﷻ watches over everything and everyone.",
      "He knows what we do in secret and in public.",
      "He protects His creation and records all that happens.",
      "Trust that Allah ﷻ is always near, caring for you and guiding you."
    ],
    reflection: "What does it mean to you that Allah ﷻ is always watching with care?",
    quizId: "al_muhaymin_quiz"
  },
  {
    id: 11,
    title: "Al-Azeez",
    arabic: "الْعَزِيز",
    meaning: "The Mighty",
    description: [
      "Al-Azeez means Allah ﷻ is strong and nothing can defeat Him.",
      "He gives honour and strength to whom He wills.",
      "He is powerful yet merciful to His servants.",
      "Knowing Allah ﷻ is Al-Azeez helps us rely on Him in every challenge."
    ],
    reflection: "When do you feel Allah ﷻ’s strength helping you?",
    quizId: "al_azeez_quiz"
  },
  {
    id: 12,
    title: "Al-Jabbaar",
    arabic: "الْجَبَّار",
    meaning: "The Compeller",
    description: [
      "Al-Jabbaar means Allah ﷻ fixes what is broken and makes things right.",
      "He is strong and able to bring everything under His command.",
      "He heals the hearts of the sad and gives comfort to those who need it.",
      "We should remember that Allah ﷻ can make every hard situation better."
    ],
    reflection: "What do you need Allah ﷻ to fix in your heart today?",
    quizId: "al_jabbaar_quiz"
  },
  {
    id: 13,
    title: "Al-Mutakabbir",
    arabic: "الْمُتَكَبِّر",
    meaning: "The Supreme in Greatness",
    description: [
      "Al-Mutakabbir means Allah ﷻ is far greater than everything He created.",
      "His greatness is perfect and full of honour, not pride like humans have.",
      "He is above all weakness, need, or comparison.",
      "Knowing Allah ﷻ is Al-Mutakabbir reminds us to stay humble and never boast."
    ],
    reflection: "What can you do today to show humility before Allah ﷻ?",
    quizId: "al_mutakabbir_quiz"
  },
  {
    id: 14,
    title: "Al-Khaaliq",
    arabic: "الْخَالِق",
    meaning: "The Creator",
    description: [
      "Al-Khaaliq means Allah ﷻ made everything from nothing.",
      "He planned every detail of the universe perfectly.",
      "Every tree, mountain, and person shows His amazing design.",
      "When we look around, we see signs of the Creator everywhere."
    ],
    reflection: "What is one thing in nature that makes you think of Allah ﷻ?",
    quizId: "al_khaaliq_quiz"
  },
  {
    id: 15,
    title: "Al-Baari'",
    arabic: "الْبَارِئ",
    meaning: "The Maker of Order",
    description: [
      "Al-Baari’ means Allah ﷻ shapes His creation in the best way.",
      "He gives each creature its right form and purpose.",
      "Nothing in the world is random; Allah ﷻ made it all fit perfectly together.",
      "This teaches us that Allah ﷻ’s wisdom is in every part of His creation."
    ],
    reflection: "How does seeing order in the world make you trust Allah ﷻ?",
    quizId: "al_baari_quiz"
  },
  {
    id: 16,
    title: "Al-Musawwir",
    arabic: "الْمُصَوِّر",
    meaning: "The Fashioner",
    description: [
      "Al-Musawwir means Allah ﷻ gives everything its special shape and beauty.",
      "Every face, colour, and sound is part of His amazing design.",
      "He made each of us unique so we can recognise His creativity.",
      "Thank Allah ﷻ for how He made you — perfectly for your purpose."
    ],
    reflection: "What do you like most about how Allah ﷻ created you?",
    quizId: "al_musawwir_quiz"
  },
  {
    id: 17,
    title: "Al-Ghaffaar",
    arabic: "الْغَفَّار",
    meaning: "The Oft-Forgiving",
    description: [
      "Al-Ghaffaar means Allah ﷻ forgives again and again.",
      "No matter how many times we sin, if we turn back sincerely, He forgives.",
      "He hides our mistakes and replaces them with good when we repent.",
      "We should also forgive others easily, just as Allah ﷻ forgives us."
    ],
    reflection: "Who can you forgive today to earn Allah ﷻ’s forgiveness?",
    quizId: "al_ghaffaar_quiz"
  },
  {
    id: 18,
    title: "Al-Qahhaar",
    arabic: "الْقَهَّار",
    meaning: "The All-Prevailing",
    description: [
      "Al-Qahhaar means Allah ﷻ has complete power over everything.",
      "Nothing can escape His will or challenge His rule.",
      "He conquers all injustice and falsehood with His truth.",
      "Remembering Al-Qahhaar gives us strength to resist wrongdoing."
    ],
    reflection: "How does knowing Allah ﷻ controls everything make you feel safe?",
    quizId: "al_qahhaar_quiz"
  },
  {
    id: 19,
    title: "Al-Wahhaab",
    arabic: "الْوَهَّاب",
    meaning: "The Bestower",
    description: [
      "Al-Wahhaab means Allah ﷻ gives gifts freely out of His kindness.",
      "He gives life, knowledge, friends, and chances to do good.",
      "Even when we forget to ask, Allah ﷻ still blesses us.",
      "We should share what we have with others to follow His example."
    ],
    reflection: "What blessing can you share with someone today?",
    quizId: "al_wahhaab_quiz"
  },
  {
    id: 20,
    title: "Ar-Razzaaq",
    arabic: "الرَّزَّاق",
    meaning: "The Provider",
    description: [
      "Ar-Razzaaq means Allah ﷻ gives every creature the food and help it needs.",
      "He provides rain for plants, homes for animals, and families for people.",
      "Even the birds in the sky depend on Him for every meal.",
      "Trust that your rizq comes only from Allah ﷻ, not from people."
    ],
    reflection: "What has Allah ﷻ provided for you today?",
    quizId: "ar_razzaaq_quiz"
  },
  {
    id: 21,
    title: "Al-Fattaah",
    arabic: "الْفَتَّاح",
    meaning: "The Opener",
    description: [
      "Al-Fattaah means Allah ﷻ opens the doors of goodness and success.",
      "He helps us find solutions when things seem impossible.",
      "He opens hearts to guidance and minds to understanding.",
      "Ask Al-Fattaah to open your heart to faith and your life to ease."
    ],
    reflection: "What do you need Allah ﷻ to open for you today?",
    quizId: "al_fattaah_quiz"
  },
  {
    id: 22,
    title: "Al-ʿAleem",
    arabic: "الْعَلِيم",
    meaning: "The All-Knowing",
    description: [
      "Al-ʿAleem means Allah ﷻ knows everything — past, present, and future.",
      "He knows our thoughts, dreams, and feelings even when we hide them.",
      "Nothing is forgotten or missed by His knowledge.",
      "Remember that Allah ﷻ knows best what is good for you."
    ],
    reflection: "How does knowing Allah ﷻ sees and knows you change your actions?",
    quizId: "al_aleem_quiz"
  },
  {
    id: 23,
    title: "Al-Qaabid",
    arabic: "الْقَابِض",
    meaning: "The Withholder",
    description: [
      "Al-Qaabid means Allah ﷻ can hold back something for a good reason.",
      "Sometimes He withholds money, success, or answers to teach patience.",
      "He never withholds out of cruelty — only out of wisdom.",
      "Be content knowing that what He delays may protect you from harm."
    ],
    reflection: "Can you trust Allah ﷻ even when you don’t get what you want?",
    quizId: "al_qaabid_quiz"
  },
  {
    id: 24,
    title: "Al-Baasit",
    arabic: "الْبَاسِط",
    meaning: "The Expander",
    description: [
      "Al-Baasit means Allah ﷻ gives freely and spreads His blessings wide.",
      "He opens His hand in generosity to whom He wills.",
      "When life feels full of blessings, it’s from His kindness.",
      "We should thank Him in times of ease and share with others."
    ],
    reflection: "Who can you share your blessings with today?",
    quizId: "al_baasit_quiz"
  },
  {
    id: 25,
    title: "Al-Khaafid",
    arabic: "الْخَافِض",
    meaning: "The Abaser",
    description: [
      "Al-Khaafid means Allah ﷻ can lower whoever He wills in status or power.",
      "He humbles the proud and reminds them that greatness belongs only to Him.",
      "He lowers people through tests so they turn back to Him with humility.",
      "We should never look down on others, because honour comes only from Allah ﷻ."
    ],
    reflection: "How can you stay humble when Allah ﷻ gives you success?",
    quizId: "al_khaafid_quiz"
  },
  {
    id: 26,
    title: "Ar-Raafiʿ",
    arabic: "الرَّافِع",
    meaning: "The Exalter",
    description: [
      "Ar-Raafiʿ means Allah ﷻ raises up whomever He wills in honour and faith.",
      "He lifts the hearts of believers and gives them respect through good deeds.",
      "He raises people who remember Him and live with honesty and kindness.",
      "Ask Allah ﷻ to raise you through knowledge, humility, and good character."
    ],
    reflection: "What good deed can lift your rank with Allah ﷻ today?",
    quizId: "ar_raafi_quiz"
  },
  {
    id: 27,
    title: "Al-Muʿizz",
    arabic: "الْمُعِزّ",
    meaning: "The Giver of Honour",
    description: [
      "Al-Muʿizz means Allah ﷻ gives respect and strength to whom He wills.",
      "He gives honour to those who obey Him and live with truth.",
      "True honour is not in money or followers but in being close to Allah ﷻ.",
      "We should seek dignity through faith and helping others."
    ],
    reflection: "How can you earn honour with Allah ﷻ instead of people?",
    quizId: "al_muizz_quiz"
  },
  {
    id: 28,
    title: "Al-Mudhill",
    arabic: "الْمُذِلّ",
    meaning: "The Humiliator",
    description: [
      "Al-Mudhill means Allah ﷻ lowers those who turn away from Him and do wrong.",
      "He removes pride and false power from those who misuse it.",
      "His justice protects the weak and reminds us that evil never wins.",
      "Fear Allah ﷻ’s displeasure and stay gentle and fair with everyone."
    ],
    reflection: "How can you avoid pride and stay near to Allah ﷻ?",
    quizId: "al_mudhill_quiz"
  },
  {
    id: 29,
    title: "As-Sameeʿ",
    arabic: "السَّمِيع",
    meaning: "The All-Hearing",
    description: [
      "As-Sameeʿ means Allah ﷻ hears every sound and every prayer.",
      "No whisper or cry is hidden from Him, even the voice in your heart.",
      "When you make duʿaa, He always hears you, even before you speak.",
      "This Name helps us speak kindly, knowing Allah ﷻ hears all words."
    ],
    reflection: "What do your words sound like to Allah ﷻ today?",
    quizId: "as_samee_quiz"
  },
  {
    id: 30,
    title: "Al-Baseer",
    arabic: "الْبَصِير",
    meaning: "The All-Seeing",
    description: [
      "Al-Baseer means Allah ﷻ sees everything clearly, no matter how small.",
      "He sees the stars in the sky and the thoughts in our hearts.",
      "Nothing is hidden from His sight, and His watch is full of mercy.",
      "We should remember Allah ﷻ sees us and try to please Him in private and public."
    ],
    reflection: "Would you do the same actions if you remembered Allah ﷻ is watching?",
    quizId: "al_baseer_quiz"
  },
  {
    id: 31,
    title: "Al-Hakam",
    arabic: "الْحَكَم",
    meaning: "The Judge",
    description: [
      "Al-Hakam means Allah ﷻ judges fairly between people.",
      "He knows every fact and never makes a mistake.",
      "His rulings are based on truth and wisdom, not anger or desire.",
      "Trust Allah ﷻ’s judgement and be fair when you deal with others."
    ],
    reflection: "How can you be fair in your choices today?",
    quizId: "al_hakam_quiz"
  },
  {
    id: 32,
    title: "Al-ʿAdl",
    arabic: "الْعَدْل",
    meaning: "The Just",
    description: [
      "Al-ʿAdl means Allah ﷻ is perfectly fair in everything He does.",
      "He rewards good and punishes wrong with full justice.",
      "Sometimes we don’t see justice right away, but Allah ﷻ never forgets.",
      "We should love justice and avoid cheating or hurting others."
    ],
    reflection: "How can you practise fairness in your daily life?",
    quizId: "al_adl_quiz"
  },
  {
    id: 33,
    title: "Al-Lateef",
    arabic: "اللَّطِيف",
    meaning: "The Gentle and Subtle",
    description: [
      "Al-Lateef means Allah ﷻ cares for us in ways we don’t even notice.",
      "He makes things easy and helps us softly without us realising.",
      "His kindness reaches our hearts through small blessings every day.",
      "We should look for Allah ﷻ’s gentle care in every situation."
    ],
    reflection: "What small blessing showed Allah ﷻ’s kindness to you today?",
    quizId: "al_lateef_quiz"
  },
  {
    id: 34,
    title: "Al-Khabeer",
    arabic: "الْخَبِير",
    meaning: "The All-Aware",
    description: [
      "Al-Khabeer means Allah ﷻ knows everything deeply and fully.",
      "He knows what is hidden in our hearts and what will happen next.",
      "His knowledge helps us see that He always chooses the best for us.",
      "We should trust His wisdom even when we don’t understand His plans."
    ],
    reflection: "When was a time Allah ﷻ knew what was best for you even if you didn’t?",
    quizId: "al_khabeer_quiz"
  },
  {
    id: 35,
    title: "Al-Haleem",
    arabic: "الْحَلِيم",
    meaning: "The Forbearing",
    description: [
      "Al-Haleem means Allah ﷻ is patient and does not punish us right away.",
      "He gives time to people so they can repent and change.",
      "His patience shows mercy even when we make mistakes.",
      "We should also be calm and forgiving with others."
    ],
    reflection: "When can you show patience like Allah ﷻ teaches?",
    quizId: "al_haleem_quiz"
  },
  {
    id: 36,
    title: "Al-ʿAzeem",
    arabic: "الْعَظِيم",
    meaning: "The Magnificent",
    description: [
      "Al-ʿAzeem means Allah ﷻ is truly great and deserves all praise.",
      "His creation, His power, and His mercy all show His greatness.",
      "When we bow in prayer and say 'Subhaana Rabbiyal-ʿAzeem', we remember this.",
      "Thinking about His greatness keeps our hearts full of awe and love."
    ],
    reflection: "What reminds you most of Allah ﷻ’s greatness?",
    quizId: "al_azeem_quiz"
  },
  {
    id: 37,
    title: "Al-Ghafoor",
    arabic: "الْغَفُور",
    meaning: "The All-Forgiving",
    description: [
      "Al-Ghafoor means Allah ﷻ forgives all sins for those who repent sincerely.",
      "He hides our faults and replaces our bad deeds with good.",
      "His forgiveness never ends, no matter how many times we return to Him.",
      "Remembering Al-Ghafoor helps us stay hopeful and forgiving of others."
    ],
    reflection: "Who can you forgive today to please Al-Ghafoor?",
    quizId: "al_ghafoor_quiz"
  },
  {
    id: 38,
    title: "Ash-Shakoor",
    arabic: "الشَّكُور",
    meaning: "The Appreciative",
    description: [
      "Ash-Shakoor means Allah ﷻ appreciates every small good deed we do.",
      "He multiplies rewards far more than we deserve.",
      "Even a smile or helping hand can earn His appreciation.",
      "We should also thank people and always thank Allah ﷻ first."
    ],
    reflection: "What can you thank Allah ﷻ for right now?",
    quizId: "ash_shakoor_quiz"
  },
  {
    id: 39,
    title: "Al-ʿAliyy",
    arabic: "الْعَلِيّ",
    meaning: "The Most High",
    description: [
      "Al-ʿAliyy means Allah ﷻ is higher than everything in power and honour.",
      "He is above all creation, perfect in every way.",
      "No one can reach His level or compare to His greatness.",
      "We raise our hands in duʿaa because He is the Most High."
    ],
    reflection: "How does remembering Allah ﷻ’s highness make you feel humble?",
    quizId: "al_aliyy_quiz"
  },
  {
    id: 40,
    title: "Al-Kabeer",
    arabic: "الْكَبِير",
    meaning: "The Most Great",
    description: [
      "Al-Kabeer means Allah ﷻ is greater than anything we can imagine.",
      "Every mountain, planet, and star shows His greatness.",
      "He is not limited by size or place—His greatness has no end.",
      "When we say 'Allah ﷻu Akbar', we remember nothing is greater than Him."
    ],
    reflection: "What reminds you to say 'Allah ﷻu Akbar' each day?",
    quizId: "al_kabeer_quiz"
  },
  {
    id: 41,
    title: "Al-Hafeedh",
    arabic: "الْحَفِيظ",
    meaning: "The Preserver",
    description: [
      "Al-Hafeedh means Allah ﷻ protects and guards everything He created.",
      "He keeps the heavens, the earth, and our hearts safe by His care.",
      "He protects us from harm and keeps our good deeds recorded.",
      "Trust that nothing can harm you unless He allows it for good reason."
    ],
    reflection: "When has Allah ﷻ protected you without you realising?",
    quizId: "al_hafeedh_quiz"
  },
  {
    id: 42,
    title: "Al-Muqeet",
    arabic: "الْمُقِيت",
    meaning: "The Nourisher",
    description: [
      "Al-Muqeet means Allah ﷻ provides strength and nourishment to everything.",
      "He gives food to every creature and power to every heart.",
      "He knows what we need most—both food for the body and faith for the soul.",
      "We should say Alhamdulillah for every meal and blessing He sends."
    ],
    reflection: "What blessing gives you strength today?",
    quizId: "al_muqeet_quiz"
  },
  {
    id: 43,
    title: "Al-Haseeb",
    arabic: "الْحَسِيب",
    meaning: "The Reckoner",
    description: [
      "Al-Haseeb means Allah ﷻ knows and keeps account of everything.",
      "He never forgets even the smallest good or bad deed.",
      "He is enough as a witness and judge for us all.",
      "This reminds us to live honestly and depend only on Him."
    ],
    reflection: "If Allah ﷻ took your account today, what good would you hope to show?",
    quizId: "al_haseeb_quiz"
  },
  {
    id: 44,
    title: "Al-Jaleel",
    arabic: "الْجَلِيل",
    meaning: "The Majestic",
    description: [
      "Al-Jaleel means Allah ﷻ is full of majesty, honour, and greatness.",
      "Everything in creation shows His beauty and power.",
      "He deserves our deepest respect and love.",
      "Remembering Al-Jaleel fills the heart with awe and gratitude."
    ],
    reflection: "How can you show respect to Allah ﷻ today?",
    quizId: "al_jaleel_quiz"
  },
  {
    id: 45,
    title: "Al-Kareem",
    arabic: "الْكَرِيم",
    meaning: "The Generous",
    description: [
      "Al-Kareem means Allah ﷻ gives without limit and never stops giving.",
      "He gives us more than we ask for and forgives us when we fail.",
      "His generosity includes guidance, food, and every blessing.",
      "We should be generous with others, just as Allah ﷻ is with us."
    ],
    reflection: "Who can you give something to today?",
    quizId: "al_kareem_quiz"
  },
  {
    id: 46,
    title: "Ar-Raqeeb",
    arabic: "الرَّقِيب",
    meaning: "The Watchful",
    description: [
      "Ar-Raqeeb means Allah ﷻ is always watching over His creation.",
      "He sees every action and intention, whether big or small.",
      "His watchfulness keeps us safe and helps us stay sincere.",
      "Remembering Ar-Raqeeb keeps us honest and mindful."
    ],
    reflection: "Would you change anything if you remembered Allah ﷻ is watching?",
    quizId: "ar_raqeeb_quiz"
  },
  {
    id: 47,
    title: "Al-Mujeeb",
    arabic: "الْمُجِيب",
    meaning: "The Responder",
    description: [
      "Al-Mujeeb means Allah ﷻ answers the prayers of those who call on Him.",
      "He listens to every duʿaa and responds in the best way and time.",
      "Sometimes the answer is yes, sometimes later, or something better.",
      "Keep asking Him because He loves when we make duʿaa often."
    ],
    reflection: "What duʿaa do you want Allah ﷻ to answer soon?",
    quizId: "al_mujeeb_quiz"
  },
  {
    id: 48,
    title: "Al-Waasiʿ",
    arabic: "الْوَاسِع",
    meaning: "The All-Encompassing",
    description: [
      "Al-Waasiʿ means Allah ﷻ’s mercy, knowledge, and gifts cover everything.",
      "No one is left out from His care and compassion.",
      "His knowledge fills every place, and His giving never ends.",
      "We should remember that Allah ﷻ’s mercy is wider than our worries."
    ],
    reflection: "How can you remember Allah ﷻ’s mercy is always enough for you?",
    quizId: "al_waasi_quiz"
  },
  {
    id: 49,
    title: "Al-Hakeem",
    arabic: "الْحَكِيم",
    meaning: "The All-Wise",
    description: [
      "Al-Hakeem means Allah ﷻ does everything with perfect wisdom.",
      "He knows exactly what is best for us, even when we don’t understand it.",
      "Every rule in the Qur’an and every event in life has a wise purpose.",
      "Trust that Allah ﷻ’s plan is always fair, kind, and full of wisdom."
    ],
    reflection: "When was a time you saw Allah ﷻ’s wisdom after waiting patiently?",
    quizId: "al_hakeem_quiz"
  },
  {
    id: 50,
    title: "Al-Wadood",
    arabic: "الْوَدُود",
    meaning: "The Loving",
    description: [
      "Al-Wadood means Allah ﷻ loves His creation and wants good for them.",
      "He shows His love through mercy, forgiveness, and blessings.",
      "He loves when we turn to Him in prayer and follow His guidance.",
      "We can show love by being kind to people and helping those in need."
    ],
    reflection: "How can you show love for Allah ﷻ and His creation today?",
    quizId: "al_wadood_quiz"
  },
  {
    id: 51,
    title: "Al-Majeed",
    arabic: "الْمَجِيد",
    meaning: "The Most Glorious",
    description: [
      "Al-Majeed means Allah ﷻ is full of glory, honour, and perfection.",
      "His actions are noble, His promises true, and His mercy endless.",
      "He gives dignity to those who remember Him and follow His way.",
      "Praising Al-Majeed fills the heart with gratitude and respect."
    ],
    reflection: "What’s one way you can honour Allah ﷻ today?",
    quizId: "al_majeed_quiz"
  },
  {
    id: 52,
    title: "Al-Baʿith",
    arabic: "الْبَاعِث",
    meaning: "The Resurrector",
    description: [
      "Al-Baʿith means Allah ﷻ will bring everyone back to life after death.",
      "He will judge all people and reward or punish with justice.",
      "He also sends life back to the earth with rain and renewal.",
      "Believing in Al-Baʿith reminds us to prepare for the Day of Judgement."
    ],
    reflection: "How does remembering the Day of Judgement affect your actions?",
    quizId: "al_baith_quiz"
  },
  {
    id: 53,
    title: "Ash-Shaheed",
    arabic: "الشَّهِيد",
    meaning: "The Witness",
    description: [
      "Ash-Shaheed means Allah ﷻ sees and witnesses everything that happens.",
      "He is always present, aware of every truth and falsehood.",
      "No one can hide from His sight or change His record.",
      "Knowing Allah ﷻ is Ash-Shaheed helps us be honest even when alone."
    ],
    reflection: "Would you act differently if you remembered Allah ﷻ is your witness?",
    quizId: "ash_shaheed_quiz"
  },
  {
    id: 54,
    title: "Al-Haqq",
    arabic: "الْحَقّ",
    meaning: "The Truth",
    description: [
      "Al-Haqq means Allah ﷻ is the only true reality.",
      "Everything else will end, but Allah ﷻ remains forever.",
      "His words, promises, and guidance are all true and never fail.",
      "Following the truth brings peace and honour in this world and the next."
    ],
    reflection: "What truth from the Qur’an inspires you most?",
    quizId: "al_haqq_quiz"
  },
  {
    id: 55,
    title: "Al-Wakeel",
    arabic: "الْوَكِيل",
    meaning: "The Trustee",
    description: [
      "Al-Wakeel means Allah ﷻ is the best One to depend on for everything.",
      "He manages all affairs perfectly and never disappoints those who trust Him.",
      "When we say 'HasbunAllah ﷻu wa niʿmal Wakeel', we put full trust in Him.",
      "Relying on Allah ﷻ makes the heart peaceful even in hard times."
    ],
    reflection: "What worry can you hand over to Allah ﷻ today?",
    quizId: "al_wakeel_quiz"
  },
  {
    id: 56,
    title: "Al-Qawiyy",
    arabic: "الْقَوِيّ",
    meaning: "The All-Strong",
    description: [
      "Al-Qawiyy means Allah ﷻ has complete strength and power.",
      "Nothing can defeat Him or make Him tired.",
      "He gives strength to those who rely on Him and obey His commands.",
      "Remember that true power belongs only to Allah ﷻ."
    ],
    reflection: "What makes you feel strong when you trust Allah ﷻ?",
    quizId: "al_qawiyy_quiz"
  },
  {
    id: 57,
    title: "Al-Mateen",
    arabic: "الْمَتِين",
    meaning: "The Firm and Steadfast",
    description: [
      "Al-Mateen means Allah ﷻ’s power is strong and unchanging.",
      "He never weakens, forgets, or gets tired.",
      "His strength holds up the heavens and earth perfectly.",
      "This reminds us to stay firm in faith and rely on Him through every test."
    ],
    reflection: "How can you stay firm in your faith this week?",
    quizId: "al_mateen_quiz"
  },
  {
    id: 58,
    title: "Al-Waliyy",
    arabic: "الْوَلِيّ",
    meaning: "The Protecting Friend",
    description: [
      "Al-Waliyy means Allah ﷻ is the close friend and protector of the believers.",
      "He helps them in every situation and guides them to what is good.",
      "He never leaves those who love Him without care.",
      "We should make Allah ﷻ our best friend through prayer and obedience."
    ],
    reflection: "What makes you feel close to Allah ﷻ as your friend?",
    quizId: "al_waliyy_quiz"
  },
  {
    id: 59,
    title: "Al-Hameed",
    arabic: "الْحَمِيد",
    meaning: "The Praiseworthy",
    description: [
      "Al-Hameed means Allah ﷻ deserves all praise for everything He does.",
      "He is perfect in His actions, generous in His gifts, and just in His rulings.",
      "Every blessing we enjoy is a reason to say 'Alhamdulillah'.",
      "Praising Allah ﷻ keeps our hearts thankful and happy."
    ],
    reflection: "What blessing will you thank Allah ﷻ for today?",
    quizId: "al_hameed_quiz"
  },
  {
    id: 60,
    title: "Al-Muhsee",
    arabic: "الْمُحْصِي",
    meaning: "The Counter",
    description: [
      "Al-Muhsee means Allah ﷻ counts and records everything in the universe.",
      "He knows the number of stars, grains of sand, and thoughts in every heart.",
      "Nothing is forgotten or lost in His knowledge.",
      "This helps us remember that every action we do is noticed by Allah ﷻ."
    ],
    reflection: "How does knowing Allah ﷻ counts all deeds affect your choices?",
    quizId: "al_muhsee_quiz"
  },
  {
    id: 61,
    title: "Al-Mubdiʿ",
    arabic: "الْمُبْدِئ",
    meaning: "The Originator",
    description: [
      "Al-Mubdiʿ means Allah ﷻ begins creation from nothing.",
      "He started the universe, life, and every new moment with perfect design.",
      "Every new idea, birth, and sunrise is a sign of His power to begin.",
      "He alone can start life and bring it back again after death."
    ],
    reflection: "What new good habit can you begin for Allah ﷻ today?",
    quizId: "al_mubdi_quiz"
  },
  {
    id: 62,
    title: "Al-Muʿeed",
    arabic: "الْمُعِيد",
    meaning: "The Restorer",
    description: [
      "Al-Muʿeed means Allah ﷻ brings things back after they are gone.",
      "He will bring every soul back to life for judgement after death.",
      "He can also restore health, hope, or happiness when we lose them.",
      "This Name gives us hope that nothing is ever truly lost with Allah ﷻ."
    ],
    reflection: "What do you want Allah ﷻ to restore for you?",
    quizId: "al_mueed_quiz"
  },
  {
    id: 63,
    title: "Al-Muhyi",
    arabic: "الْمُحْيِي",
    meaning: "The Giver of Life",
    description: [
      "Al-Muhyi means Allah ﷻ gives life to all living things.",
      "He brings the dead earth to life with rain and gives us breath every day.",
      "He also gives spiritual life to hearts through faith and Qur’an.",
      "We should thank Him every morning for the gift of life and belief."
    ],
    reflection: "What makes your heart feel alive with faith?",
    quizId: "al_muhyi_quiz"
  },
  {
    id: 64,
    title: "Al-Mumeet",
    arabic: "الْمُمِيت",
    meaning: "The Giver of Death",
    description: [
      "Al-Mumeet means Allah ﷻ takes life away when the time is right.",
      "Death is not an end, but a step to the next life He promised.",
      "He gives life and death with wisdom and perfect timing.",
      "Remembering Al-Mumeet helps us live each day with purpose and care."
    ],
    reflection: "If today were your last day, what good would you want to do?",
    quizId: "al_mumeet_quiz"
  },
  {
    id: 65,
    title: "Al-Hayy",
    arabic: "الْحَيّ",
    meaning: "The Ever-Living",
    description: [
      "Al-Hayy means Allah ﷻ is alive forever and never dies.",
      "He doesn’t sleep, forget, or become tired.",
      "All life comes from Him, and He remains while everything else fades.",
      "When we feel weak or afraid, we remember Allah ﷻ is always living and watching."
    ],
    reflection: "How does it comfort you that Allah ﷻ never dies or forgets?",
    quizId: "al_hayy_quiz"
  },
  {
    id: 66,
    title: "Al-Qayyoom",
    arabic: "الْقَيُّوم",
    meaning: "The Self-Sustaining",
    description: [
      "Al-Qayyoom means Allah ﷻ takes care of everything without needing help.",
      "He provides for all creation and keeps the universe running perfectly.",
      "Nothing can exist or continue without Him.",
      "We depend on Him for every breath and every blessing."
    ],
    reflection: "How can you rely more on Allah ﷻ each day?",
    quizId: "al_qayyoom_quiz"
  },
  {
    id: 67,
    title: "Al-Waajid",
    arabic: "الْوَاجِد",
    meaning: "The Perceiver",
    description: [
      "Al-Waajid means Allah ﷻ finds and knows everything without searching.",
      "He knows what exists, what is hidden, and what is missing.",
      "He never loses anything and nothing escapes His awareness.",
      "We can always turn to Him when we feel lost or forgotten."
    ],
    reflection: "How can you remember that Allah ﷻ always knows where you are?",
    quizId: "al_waajid_quiz"
  },
  {
    id: 68,
    title: "Al-Maajid",
    arabic: "الْمَاجِد",
    meaning: "The Most Noble",
    description: [
      "Al-Maajid means Allah ﷻ is full of goodness, generosity, and honour.",
      "His greatness is seen in His kindness to creation and His forgiveness.",
      "He is noble in all He gives and all He does.",
      "We should act nobly and treat others with dignity and respect."
    ],
    reflection: "What’s one noble act you can do today for Allah ﷻ’s sake?",
    quizId: "al_maajid_quiz"
  },
  {
    id: 69,
    title: "Al-Waahid",
    arabic: "الْوَاحِد",
    meaning: "The One",
    description: [
      "Al-Waahid means Allah ﷻ is One, with no partner or equal.",
      "Everything in creation shows that there is only one Creator.",
      "He alone deserves our worship and love.",
      "Believing in His oneness keeps our hearts pure from shirk."
    ],
    reflection: "How can you show your belief that Allah ﷻ is One today?",
    quizId: "al_waahid_quiz"
  },
  {
    id: 70,
    title: "As-Samad",
    arabic: "الصَّمَد",
    meaning: "The Eternal Refuge",
    description: [
      "As-Samad means Allah ﷻ is the One we turn to for every need.",
      "He does not depend on anyone, but everyone depends on Him.",
      "He is always ready to listen, help, and provide.",
      "When life feels hard, call on As-Samad and trust He will answer."
    ],
    reflection: "What do you need from Allah ﷻ right now?",
    quizId: "as_samad_quiz"
  },
  {
    id: 71,
    title: "Al-Qaadir",
    arabic: "الْقَادِر",
    meaning: "The Able",
    description: [
      "Al-Qaadir means Allah ﷻ has full power to do whatever He wills.",
      "Nothing is too easy or too hard for Him.",
      "He can change any situation in a moment.",
      "We should ask Him for anything, because His power has no limit."
    ],
    reflection: "What big thing do you need Allah ﷻ’s help with?",
    quizId: "al_qaadir_quiz"
  },
  {
    id: 72,
    title: "Al-Muqtadir",
    arabic: "الْمُقْتَدِر",
    meaning: "The All-Powerful",
    description: [
      "Al-Muqtadir means Allah ﷻ’s power controls every part of creation.",
      "He arranges everything with strength, wisdom, and care.",
      "He gives and takes, raises and lowers, as He wills.",
      "Knowing Al-Muqtadir helps us feel safe in Allah ﷻ’s perfect plan."
    ],
    reflection: "How can you show trust in Allah ﷻ’s power this week?",
    quizId: "al_muqtadir_quiz"
  },
  {
    id: 73,
    title: "Al-Muqaddim",
    arabic: "الْمُقَدِّم",
    meaning: "The One Who Brings Forward",
    description: [
      "Al-Muqaddim means Allah ﷻ brings things forward at the perfect time.",
      "He gives honour, chances, and success to those He chooses.",
      "He advances His servants through faith, patience, and good deeds.",
      "This reminds us that no one can come forward unless Allah ﷻ allows it."
    ],
    reflection: "What can you do today to move forward closer to Allah ﷻ?",
    quizId: "al_muqaddim_quiz"
  },
  {
    id: 74,
    title: "Al-Mu’akhkhir",
    arabic: "الْمُؤَخِّر",
    meaning: "The One Who Delays",
    description: [
      "Al-Mu’akhkhir means Allah ﷻ delays things for wise reasons.",
      "Sometimes He holds back a blessing or answer so we grow in patience.",
      "He never forgets — His timing is always perfect.",
      "Trust that every delay has goodness hidden in it."
    ],
    reflection: "What delayed thing in your life can you now see was for your good?",
    quizId: "al_muakhkhir_quiz"
  },
  {
    id: 75,
    title: "Al-Awwal",
    arabic: "الْأَوَّل",
    meaning: "The First",
    description: [
      "Al-Awwal means Allah ﷻ existed before everything else.",
      "Nothing came before Him; He was always there and will always be.",
      "All creation began by His command.",
      "When we remember Al-Awwal, we know our lives start and end with Allah ﷻ."
    ],
    reflection: "How can you begin your day by remembering Al-Awwal?",
    quizId: "al_awwal_quiz"
  },
  {
    id: 76,
    title: "Al-Aakhir",
    arabic: "الْآخِر",
    meaning: "The Last",
    description: [
      "Al-Aakhir means Allah ﷻ will remain after everything ends.",
      "Everything else will pass away, but Allah ﷻ will always exist.",
      "He is the final goal of every heart that seeks truth.",
      "Remembering Al-Aakhir helps us focus on what really lasts — the Hereafter."
    ],
    reflection: "What lasting good deed can you do today for Allah ﷻ’s sake?",
    quizId: "al_aakhir_quiz"
  },
  {
    id: 77,
    title: "Az-Zaahir",
    arabic: "الظَّاهِر",
    meaning: "The Most High and Apparent",
    description: [
      "Az-Zaahir means Allah ﷻ’s power and signs are clear in everything around us.",
      "His greatness is visible in the sun, stars, and every part of creation.",
      "Nothing is above Him or outside His knowledge.",
      "Seeing His signs makes our faith stronger and hearts more thankful."
    ],
    reflection: "What’s one sign of Allah ﷻ’s greatness you can see today?",
    quizId: "az_zaahir_quiz"
  },
  {
    id: 78,
    title: "Al-Baatin",
    arabic: "الْبَاطِن",
    meaning: "The Hidden and Near",
    description: [
      "Al-Baatin means Allah ﷻ is close and knows what is hidden inside us.",
      "He is near to our hearts even if we cannot see Him.",
      "He helps us secretly in ways we may not notice.",
      "This reminds us to keep our hearts pure, since Allah ﷻ sees within."
    ],
    reflection: "How can you make your heart more sincere today?",
    quizId: "al_baatin_quiz"
  },
  {
    id: 79,
    title: "Al-Waali",
    arabic: "الْوَالِي",
    meaning: "The Governor",
    description: [
      "Al-Waali means Allah ﷻ manages and takes care of all affairs.",
      "He organises everything in the universe perfectly.",
      "Nothing happens without His will and plan.",
      "Knowing Al-Waali helps us trust that Allah ﷻ is taking care of our lives."
    ],
    reflection: "What worry can you leave in Allah ﷻ’s care today?",
    quizId: "al_waali_quiz"
  },
  {
    id: 80,
    title: "Al-Mutaʿaali",
    arabic: "الْمُتَعَالِي",
    meaning: "The Supremely Exalted",
    description: [
      "Al-Mutaʿaali means Allah ﷻ is far above any imperfection or weakness.",
      "He is higher than anything we can imagine or describe.",
      "His greatness cannot be compared to anyone or anything.",
      "Remembering this keeps our hearts full of awe and respect."
    ],
    reflection: "How can you show respect for Allah ﷻ’s greatness?",
    quizId: "al_mutaali_quiz"
  },
  {
    id: 81,
    title: "Al-Barr",
    arabic: "الْبَرّ",
    meaning: "The Most Kind and Good",
    description: [
      "Al-Barr means Allah ﷻ is kind and gentle with His servants.",
      "He gives us blessings even when we forget to thank Him.",
      "He forgives us, feeds us, and covers our mistakes with mercy.",
      "This Name reminds us to be kind and generous like Allah ﷻ is."
    ],
    reflection: "What kind thing can you do today for someone else?",
    quizId: "al_barr_quiz"
  },
  {
    id: 82,
    title: "At-Tawwaab",
    arabic: "التَّوَّاب",
    meaning: "The Accepter of Repentance",
    description: [
      "At-Tawwaab means Allah ﷻ welcomes those who turn back to Him after mistakes.",
      "He loves when we repent sincerely, no matter how many times we fail.",
      "He turns our sins into good deeds when our hearts truly change.",
      "Repenting brings us closer to Allah ﷻ and fills us with peace."
    ],
    reflection: "What do you want to repent to Allah ﷻ for today?",
    quizId: "at_tawwaab_quiz"
  },
  {
    id: 83,
    title: "Al-Muntaqim",
    arabic: "الْمُنْتَقِم",
    meaning: "The Avenger",
    description: [
      "Al-Muntaqim means Allah ﷻ gives justice to the wronged and punishes the oppressors.",
      "He delays punishment sometimes to give chances for repentance.",
      "His justice is always fair and based on full knowledge.",
      "This Name helps us forgive others and trust Allah ﷻ’s judgement instead of seeking revenge."
    ],
    reflection: "How can you let go of anger and trust Allah ﷻ’s justice?",
    quizId: "al_muntaqim_quiz"
  },
  {
    id: 84,
    title: "Al-ʿAfuww",
    arabic: "الْعَفُوّ",
    meaning: "The Pardoner",
    description: [
      "Al-ʿAfuww means Allah ﷻ erases sins completely when we repent.",
      "He doesn’t just forgive — He removes all trace of the sin as if it never happened.",
      "He loves to pardon, especially when we forgive others too.",
      "This Name reminds us to be soft-hearted and let go of grudges."
    ],
    reflection: "Who can you forgive completely today?",
    quizId: "al_afuww_quiz"
  },
  {
    id: 85,
    title: "Ar-Ra’oof",
    arabic: "الرَّؤُوف",
    meaning: "The Most Compassionate",
    description: [
      "Ar-Ra’oof means Allah ﷻ is very kind and gentle with His servants.",
      "His compassion reaches us in every blessing, comfort, and act of mercy.",
      "Even when we disobey, He still gives us chances to come back.",
      "This reminds us to treat others with care and gentleness too."
    ],
    reflection: "How can you show more kindness like Allah ﷻ today?",
    quizId: "ar_raoof_quiz"
  },
  {
    id: 86,
    title: "Maalik-ul-Mulk",
    arabic: "مَالِكُ الْمُلْك",
    meaning: "Owner of All Dominion",
    description: [
      "Maalik-ul-Mulk means Allah ﷻ owns everything in existence.",
      "He gives and takes power, wealth, and honour to whom He wills.",
      "He rules all creation perfectly and with wisdom.",
      "Knowing this helps us stay humble and grateful for what we have."
    ],
    reflection: "How can you show gratitude to Allah ﷻ, the true Owner of all things?",
    quizId: "maalik_ul_mulk_quiz"
  },
  {
    id: 87,
    title: "Dhul-Jalaali wal-Ikraam",
    arabic: "ذُو الْجَلَالِ وَالْإِكْرَام",
    meaning: "Lord of Majesty and Honour",
    description: [
      "Dhul-Jalaali wal-Ikraam means Allah ﷻ is full of majesty, power, and generosity.",
      "He deserves our deepest respect and love.",
      "His greatness and kindness are beyond our imagination.",
      "We glorify Him in prayer and thank Him for His endless mercy."
    ],
    reflection: "How can you honour Allah ﷻ in your daily actions?",
    quizId: "dhul_jalaali_wal_ikraam_quiz"
  },
  {
    id: 88,
    title: "Al-Muqsit",
    arabic: "الْمُقْسِط",
    meaning: "The Just and Fair",
    description: [
      "Al-Muqsit means Allah ﷻ treats everyone with complete fairness.",
      "He gives everyone what they deserve, perfectly balanced.",
      "He never wrongs anyone and rewards every bit of good we do.",
      "We should try to be fair and never take more than our share."
    ],
    reflection: "When can you be more fair or honest today?",
    quizId: "al_muqsit_quiz"
  },
  {
    id: 89,
    title: "Al-Jaamiʿ",
    arabic: "الْجَامِع",
    meaning: "The Gatherer",
    description: [
      "Al-Jaamiʿ means Allah ﷻ gathers people, hearts, and events together for a purpose.",
      "He will gather everyone on the Day of Judgement with perfect justice.",
      "He brings friends, families, and believers together out of His mercy.",
      "This Name reminds us that Allah ﷻ unites us through faith."
    ],
    reflection: "Who can you bring closer together for Allah ﷻ’s sake?",
    quizId: "al_jaami_quiz"
  },
  {
    id: 90,
    title: "Al-Ghaniyy",
    arabic: "الْغَنِيّ",
    meaning: "The Self-Sufficient",
    description: [
      "Al-Ghaniyy means Allah ﷻ needs nothing from anyone.",
      "He is perfect and complete, while we depend on Him for everything.",
      "He gives freely without losing anything Himself.",
      "This reminds us to depend on Allah ﷻ, not people or things."
    ],
    reflection: "What can you thank Allah ﷻ for giving you even though He needs nothing?",
    quizId: "al_ghaniyy_quiz"
  },
  {
    id: 91,
    title: "Al-Mughni",
    arabic: "الْمُغْنِي",
    meaning: "The Enricher",
    description: [
      "Al-Mughni means Allah ﷻ gives wealth, knowledge, and contentment to whom He wills.",
      "He can make a poor person rich in heart or body with ease.",
      "True richness is being happy with what Allah ﷻ gives.",
      "We should ask Allah ﷻ to make us rich in faith and good deeds."
    ],
    reflection: "What makes you feel truly rich inside?",
    quizId: "al_mughni_quiz"
  },
  {
    id: 92,
    title: "Al-Maaniʿ",
    arabic: "الْمَانِع",
    meaning: "The Preventer",
    description: [
      "Al-Maaniʿ means Allah ﷻ protects us by stopping harm or wrong things from happening.",
      "Sometimes we don’t get what we want because Allah ﷻ is saving us from it.",
      "He blocks evil and guides us toward what is best.",
      "Be grateful even when things don’t go your way — Allah ﷻ knows why."
    ],
    reflection: "What disappointment might have been Allah ﷻ’s protection?",
    quizId: "al_maani_quiz"
  },
  {
    id: 93,
    title: "Ad-Daarr",
    arabic: "الضَّارّ",
    meaning: "The Bringer of Harm",
    description: [
      "Ad-Daarr means Allah ﷻ allows harm or hardship only for a wise reason.",
      "Every pain or test teaches us patience and makes us stronger.",
      "He never harms without mercy or justice.",
      "We should be patient, knowing Allah ﷻ brings good from every trial."
    ],
    reflection: "How have your hard times brought you closer to Allah ﷻ?",
    quizId: "ad_daarr_quiz"
  },
  {
    id: 94,
    title: "An-Naafiʿ",
    arabic: "النَّافِع",
    meaning: "The Giver of Benefit",
    description: [
      "An-Naafiʿ means Allah ﷻ gives all goodness and benefit to His creation.",
      "Every cure, success, and blessing comes only from Him.",
      "He helps His servants in ways they don’t expect.",
      "We should always ask Him for benefit and protection from harm."
    ],
    reflection: "What good thing has Allah ﷻ brought into your life recently?",
    quizId: "an_naafi_quiz"
  },
  {
    id: 95,
    title: "An-Noor",
    arabic: "النُّور",
    meaning: "The Light",
    description: [
      "An-Noor means Allah ﷻ is the light of the heavens and the earth.",
      "He guides people from darkness to truth and gives light to hearts.",
      "His light shows in the Qur’an, in knowledge, and in goodness.",
      "Ask Allah ﷻ to fill your life and heart with His light."
    ],
    reflection: "How can you bring Allah ﷻ’s light into someone’s day?",
    quizId: "an_noor_quiz"
  },
  {
    id: 96,
    title: "Al-Haadi",
    arabic: "الْهَادِي",
    meaning: "The Guide",
    description: [
      "Al-Haadi means Allah ﷻ guides people to truth and the straight path.",
      "He teaches us through the Qur’an, the Prophet ﷺ, and signs in creation.",
      "He opens the hearts of those who seek His guidance.",
      "We should always ask Al-Haadi to keep us firm on the right path."
    ],
    reflection: "What can you do to follow Allah ﷻ’s guidance better today?",
    quizId: "al_haadi_quiz"
  },
  {
    id: 97,
    title: "Al-Badeeʿ",
    arabic: "الْبَدِيع",
    meaning: "The Incomparable Creator",
    description: [
      "Al-Badeeʿ means Allah ﷻ creates in new and perfect ways, unlike anyone else.",
      "He made the universe without any example before it.",
      "Every sunset, leaf, and soul shows His unique creativity.",
      "This Name inspires us to appreciate beauty and think deeply about His creation."
    ],
    reflection: "What part of nature reminds you of Allah ﷻ’s creativity?",
    quizId: "al_badee_quiz"
  },
  {
    id: 98,
    title: "Al-Baaqi",
    arabic: "الْبَاقِي",
    meaning: "The Everlasting",
    description: [
      "Al-Baaqi means Allah ﷻ will never end or fade away.",
      "Everything else in life passes, but Allah ﷻ remains forever.",
      "This helps us remember to focus on actions that last into the Hereafter.",
      "Say 'Laa ilaaha illa Allah ﷻ' — the lasting words that lead to Jannah."
    ],
    reflection: "What good deeds can you do that will last forever?",
    quizId: "al_baaqi_quiz"
  },
  {
    id: 99,
    title: "Al-Waarith",
    arabic: "الْوَارِث",
    meaning: "The Inheritor",
    description: [
      "Al-Waarith means Allah ﷻ remains after everyone and everything passes away.",
      "He inherits the earth and all that was ever owned.",
      "He gives and takes as He wills, but He never loses anything.",
      "We should live as caretakers, not owners, using blessings wisely."
    ],
    reflection: "How can you use what you have in ways that please Allah ﷻ?",
    quizId: "al_waarith_quiz"
  },
  {
    id: 100,
    title: "Ad-Dayyan",
    arabic: "الديّان",
    meaning: "The Recompenser / The Supreme Judge",
    description: [
      "Ad-Dayyan is an Islamic name for Allah ﷻ, meaning 'The Recompenser' or 'The Supreme Judge'. It refers to His role as the ultimate judge who will reward or punish people for their deeds, based on perfect justice.",
      "The name has not appeared in the Qur'an but is mentioned in the Sunnah (sayings and actions of the Prophet Muhammad ﷺ).",
      "Allah ﷻ is the one who will perfectly recompense His servants on the Day of Judgement—rewarding good deeds and punishing bad ones, or pardoning them according to His will.",
      "This name embodies His perfect justice, meaning He will reward good deeds and punish bad ones according to His will with fairness and wisdom."
    ],
    reflection: "How does knowing Allah ﷻ is Ad-Dayyan inspire you to be more just and accountable in your actions?",
    quizId: "ad_dayyan_quiz"
  },
  {
    id: 101,
    title: "As-Saboor",
    arabic: "الصَّبُور",
    meaning: "The Patient",
    description: [
      "As-Saboor means Allah ﷻ is never in a hurry and gives everyone time to change.",
      "He delays punishment out of mercy and loves those who are patient.",
      "He teaches us to wait calmly for what is best.",
      "Patience shows trust in Allah ﷻ’s plan and timing."
    ],
    reflection: "What can you be more patient about today?",
    quizId: "as_saboor_quiz"
  },
  {
    id: 102,
    title: "Al-Jaabir",
    arabic: "الْجَابِر",
    meaning: "The Restorer of the Broken",
    description: [
      "Al-Jaabir means Allah ﷻ heals hearts and mends what is broken.",
      "He fixes sadness, pain, and loss with His care and love.",
      "He helps those who are hurt find peace again.",
      "When you feel broken, call upon Al-Jaabir — He will make you whole."
    ],
    reflection: "Whose heart can you help heal today?",
    quizId: "al_jaabir_quiz"
  },
  {
    id: 103,
    title: "Al-Qaadir-ul-Muqtadir",
    arabic: "الْقَادِرُ الْمُقْتَدِر",
    meaning: "The All-Powerful, Perfect in Ability",
    description: [
      "This combined Name shows Allah ﷻ’s total control and perfect strength.",
      "He plans, creates, and manages everything with wisdom and ease.",
      "He is never tired or unsure — His power is complete.",
      "This reminds us to rely on Allah ﷻ alone for strength and success."
    ],
    reflection: "What situation do you need Allah ﷻ’s power to handle?",
    quizId: "al_qaadir_muqtadir_quiz"
  },
  {
    id: 104,
    title: "Al-Mannaan",
    arabic: "الْمَنَّان",
    meaning: "The Bestower of Favours",
    description: [
      "Al-Mannaan means Allah ﷻ gives endless blessings and gifts without being asked.",
      "Every breath, every smile, and every chance to worship is from Him.",
      "He gives without expecting anything back, only our gratitude and obedience.",
      "This reminds us to always say 'Alhamdulillah' for His many favours."
    ],
    reflection: "What is one blessing from Allah ﷻ you’re thankful for right now?",
    quizId: "al_mannaan_quiz"
  }
  ];

  export default namesOfAllahLessonsFull;